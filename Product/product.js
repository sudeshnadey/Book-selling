function fetchDataFromAPI() {
  fetch('http://api.bhattacharjeesolution.in/book/api/product/admin-show.php', {
    headers: {
    ...
    bannerTableBody.removeChild(bannerTableBody.firstChild);
      ...
      data.forEach(function (item) {
          ...
          row.innerHTML = `
              ...
              <td>
              <img src="https://api.bhattacharjeesolution.in/book/api/images/${item.image.replace(' ', '%20')}" alt="" style="width: calc(80% - 30px); flex: 5; object-fit: cover; max-width: 100px; max-height: 100px;" />
              </td>
              <td>
                  <button class="edit-button" style="border: none"  data-coupon='${JSON.stringify(
                    ...
                    <button style="border: none; color: red" onclick="deleteBanner(${item.id})">
...
function editBanner(bannerData) {
  ...
  bannerImagePreInput.src = `https://api.bhattacharjeesolution.in/book/api/images/${bannerData.image.replace(' ', '%20')}` 
  ...
  if (!isSubmitEventListenerAdded) {
    ...
    editForm.addEventListener("submit", async function (e) {
      ...
      console.log("Form submitted:", bannerNameInput.value, bannerImageInput.files[0]);
      ...
      try {
        const response = await fetch("https://api.bhattacharjeesolution.in/book/api/edit-banner.php", {
          method: "POST",
          body: formData,
        });
